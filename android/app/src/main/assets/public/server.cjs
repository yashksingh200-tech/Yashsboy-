...
async function startServer() {
  ...
  const requireAuth = async (req, res, next) => {
    ...
    if (firebaseAdminApp && token.length > 50) {
      try {
        const decodedToken = await (0, import_auth.getAuth)(firebaseAdminApp).verifyIdToken(token);
        verifiedUid = decodedToken.uid;
      } catch (tokenErr) {
        console.warn("[Auth] Firebase ID Token verification fallback:", tokenErr.message);
      }
    }
  ...
  async function callGroqChatCompletion(options) {
    ...
    const finalUserContent = userContentParts.length > 0 ? typeof userContentParts[0] === "string" ? userContentParts[0] : model.includes("vision") ? userContentParts : options.message || "" : options.message || "";
    messages.push({
      role: "user",
  ...
  async function callOpenAIChatCompletion(options) {
    ...
    const finalUserContent = userContentParts.length > 0 ? userContentParts.length === 1 && userContentParts[0].type === "text" ? userContentParts[0].text : userContentParts : options.message || "";
    messages.push({
      role: "user",
      content: finalUserContent
    });
    ...
    try {
      ...
      if (sarvamKey) {
        try {
          ...
          const paceVal = typeof speedRate === "number" && speedRate > 0 ? speedRate : 1;
      ...
      try {
        ...
        const isQuotaError = gemErr?.status === 429 || errMsg.includes("429") || errMsg.includes("quota") || errMsg.includes("RESOURCE_EXHAUSTED");
        if (isQuotaError) {
...
- GLOBAL MULTILINGUAL FLUENCY:
  * If ${userName} writes in Spanish ("\xA1Hola! \xBFC\xF3mo est\xE1s hoy?"), reply in warm, natural Spanish ("\xA1Hola! Me alegro de hablar contigo. \xBFC\xF3mo va tu d\xEDa?").
  * If ${userName} writes in French ("Bonjour! Comment vas-tu?"), reply in fluent, everyday French ("Bonjour! Je vais tr\xE8s bien, merci. Et toi, comment te sens-tu aujourd'hui?").
  * If ${userName} writes in German ("Hallo! Wie geht es dir?"), reply in warm German ("Hallo! Sch\xF6n, von dir zu h\xF6ren. Wie war dein Tag bis jetzt?").
...
- If ${userName} expresses severe distress, self-harm thoughts, suicidal ideation, or crisis-level emotions, respond with deep compassion and warmth, gently encourage reaching out to a trusted loved one or professional support helpline (such as 988 Crisis Lifeline, Vandrevala Foundation Helpline 9999 666 555, or local emergency services). Never offer false reassurance or pretend to replace professional medical or mental health help.`;
      ...
      if (!replyText) {
        ...
        replyText = isHindiInput ? `Hey ${userName}! Main aapki AI companion ${name} hoon. Main bilkul sun rahi hoon, batao aaj kaisa feel ho raha hai?` : `I hear you loud and clear, ${userName}! I'm ${name}, your Ferio Heart AI Companion. How are you feeling right now?`;
...
Current Mood: "${mood || "neutral"}"`;
  ...
  app.post("/api/weekly-insights", requireAuth, async (req, res) => {
    try {
...
User's Active Goals: ${goalsText}`,
  ...
  app.post("/api/user/delete-data", requireAuth, (req, res) => {
    try {
  ...
  app.get(["/privacy", "/privacy/", "/privacy.html"], (req, res) => {
    const distPrivacy = import_path.default.join(process.cwd(), "dist", "privacy.html");
    const publicPrivacy = import_path.default.join(process.cwd(), "public", "privacy.html");
    if (import_fs.default.existsSync(distPrivacy)) {
